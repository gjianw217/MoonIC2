...
{
    rt_size_t i = 0;
    rt_size_t block = size/PAGE_SIZE;
    ...
    while(block--)
    {
        w25qxx_page_write(pos+i*spi_flash_device.geometry.bytes_per_sector,//(pos + i)*spi_flash_device.geometry.bytes_per_sector,//4096
                          ptr);
        ptr += PAGE_SIZE;
        i++;
    }
...
rt_err_t w25qxx_init(const char * flash_device_name, const char * spi_device_name)
{
    ...
    /* init flash */
    {
        ...
        /* read flash id */
			...
			{
        cmd = CMD_JEDEC_ID;
        rt_spi_send_then_recv(spi_flash_device.rt_spi_device, &cmd, 1, id_recv, 5);
				rt_thread_delay(5);
			}
...
int  rt_flash_init(void)
{
	...
	rt_device_open(&spi_flash_device.flash_device,RT_DEVICE_OFLAG_RDWR);
	
	return 0;
}
int flash_init_flg=0;
int  rt_flash_read(rt_base_t addr)
{
	...
	memset(buff,0,256);
	rt_device_read(&spi_flash_device.flash_device,addr,buff,256);
	for(i=0;i<256;i++)
	{
		rt_kprintf("%2x ",buff[i]);
		if((i+1)%16==0)
			rt_kprintf("\r\n");
	}
	/*rt_device_close(&spi_flash_device.flash_device);
	rt_pin_write(50,0);
	rt_pin_write(51,0);
	rt_pin_mode(50,PIN_MODE_INPUT);//nConfig
	rt_pin_mode(51,PIN_MODE_INPUT);//nCE*/
   return 0;
}
FINSH_FUNCTION_EXPORT_ALIAS(rt_flash_read, flashRead, read from flash);
...
int  rt_flash_erase()
{
    ...
    rt_spi_send(spi_flash_device.rt_spi_device, send_buffer, 1);
	  ...
	  send_buffer[0] = CMD_ERASE_full;
    rt_spi_send(spi_flash_device.rt_spi_device, send_buffer, 1);
	return 0;
}

FINSH_FUNCTION_EXPORT_ALIAS(rt_flash_erase, flashErase, erase from flash);
